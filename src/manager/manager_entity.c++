...
manager_entity::manager_entity(sf::Vector2u window_size, float &ws, float &wa,
  ...
  for (uint16_t i = 0; i < 100; i++) {
    ...
    m_registry.emplace<Components::Movement::Position>(
        entity, static_cast<float>(rand() % m_window_size.x),
        static_cast<float>(rand() % m_window_size.y));
    m_registry.emplace<Components::Movement::Velocity>(
        entity, static_cast<float>(rand() % 400) - 200.f,
