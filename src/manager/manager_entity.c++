...
manager_entity::manager_entity(sf::Vector2u window_size, float &ws, float &wa, float &wc, float &vision) {
    ...
    for (uint16_t i = 0; i < 200; i++) {
        ...
        m_registry.emplace<components::position>(entity, static_cast<float>(rand() % m_window_size.x),
                                                 static_cast<float>(rand() % m_window_size.y));
        m_registry.emplace<components::velocity>(entity, static_cast<float>(rand() % 400) - 200.f,
                                                 static_cast<float>(rand() % 400) - 200.f);
        m_registry.emplace<components::acceleration>(entity, 0.f, 0.f);
