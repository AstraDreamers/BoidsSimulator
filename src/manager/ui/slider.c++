...
namespace ui {
    slider::slider(float &value, const std::pair<float, float> &clamp)
    ...
    auto slider::set_position(const sf::Vector2f position) -> void {
        position_ = position;
    ...
    auto slider::set_size(const sf::Vector2f size) -> void {
        size_ = size;
    ...
    auto slider::set_color_inactive(const sf::Color color) -> void { color_inactive_ = color; }

    auto slider::set_color_active(const sf::Color color) -> void { color_active_ = color; }

    auto slider::set_color_background(const sf::Color color) -> void {
