...
void slider::update_geometry() {
  ...
  for (size_t i = 0; i < m_circle_segments; ++i) {
    ...
    m_vertices[offset++] =
        sf::Vertex(left_cap_center + sf::Vector2f(m_lut_cos[i] * half_y, m_lut_sin[i] * half_y), current_active_color);
  ...
  for (size_t i = 0; i < m_circle_segments; ++i) {
    ...
    m_vertices[offset++] =
        sf::Vertex(right_cap_center + sf::Vector2f(m_lut_cos[i] * half_y, m_lut_sin[i] * half_y), m_color_background);
    m_vertices[offset++] = sf::Vertex(
  ...
  for (size_t i = 0; i < m_circle_segments; ++i) {
    ...
    m_vertices[offset++] = sf::Vertex(
        thumb_center + sf::Vector2f(m_lut_cos[i] * thumb_radius, m_lut_sin[i] * thumb_radius), current_active_color);
    m_vertices[offset++] =
